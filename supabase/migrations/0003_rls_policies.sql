...
create policy "requests are readable by requester"
  on blood_requests for select using (auth.uid() = requester_id);
create policy "requests are writable by requester"
  on blood_requests for all using (auth.uid() = requester_id) with check (auth.uid() = requester_id);
...
create policy "notifications are visible to involved users"
  on notifications for select using (
    ...
    or exists (select 1 from blood_requests r where r.id = request_id and r.requester_id = auth.uid())
  );
