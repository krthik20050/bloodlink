...
The Telegram bot exposes `/start`, `/help`, `/status`, `/cancel`, and `/disconnect`. `/start` presents donor and requester web entry points; `/status` and `/disconnect` operate on a donor profile only when its verified chat ID is already linked. Configure the bot webhook with `TELEGRAM_WEBHOOK_SECRET`; the first received message configures the command menu through Telegram.
