...
Administrative access is an explicit Supabase-backed authorization record, not a shared frontend password. Deploy migration `0006_admin_authorization.sql`, then insert an authenticated user's UUID into `public.admin_users` with an `ADMIN` or `OPS` role and `is_active = true`. Every admin route verifies the Supabase session and the active record server-side before using the service-role client.
